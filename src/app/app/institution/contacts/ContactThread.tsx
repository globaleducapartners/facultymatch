    ...
    <div className={`bg-white rounded-2xl border shadow-sm transition-all ${isArchived ? "opacity-60 border-gray-100" : isReplied ? "border-green-100" : "border-gray-100 hover:border-blue-100"}`}>
      ...
      {expanded && (
        <div className="px-5 pb-5 space-y-4 border-t border-gray-50 pt-4">
          {/* Institution message bubble (right) */}
          <div className="flex flex-col items-end gap-2">
            <div className="flex items-center gap-2 text-[10px] text-gray-400 font-bold uppercase tracking-widest">
              <span>{institutionName}</span>
              <span>·</span>
              <span>{new Date(contact.created_at).toLocaleDateString("es-ES", { day: "numeric", month: "long" })}</span>
            </div>
            <div className="max-w-[80%] bg-talentia-blue text-white p-4 rounded-2xl rounded-tr-sm">
              {/* Meta tags */}
              ...
              <p className="text-sm font-medium leading-relaxed">{contact.message}</p>
            </div>
          </div>
            ...
            <div className="flex flex-col items-start gap-2">
              <div className="flex items-center gap-2 text-[10px] text-gray-400 font-bold uppercase tracking-widest">
                ...
                {contact.replied_at && (
                  <>
                    <span>·</span>
                    <span>{new Date(contact.replied_at).toLocaleDateString("es-ES", { day: "numeric", month: "long" })}</span>
                  </>
                )}
              </div>
