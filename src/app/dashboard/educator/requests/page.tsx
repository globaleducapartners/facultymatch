...
import { Mail, Search, Filter, Archive, CheckCircle2, MoreVertical, Building2, MapPin, Clock } from "lucide-react";
...
export default async function RequestsPage() {
  ...
  async function updateStatus(id: string, status: string) {
    ...
    revalidatePath("/dashboard/educator/requests");
  }
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <Tabs defaultValue="pending" className="space-y-6">
        ...
        <TabsContent value="pending">
          <Card className="border-none shadow-sm rounded-2xl overflow-hidden">
            <CardContent className="p-0 divide-y divide-gray-50">
              {requests && requests.filter(r => r.status === 'sent').length > 0 ? (
                requests.filter(r => r.status === 'sent').map((req: { 
                  id: string; 
                  created_at: string; 
                  message?: string; 
                  status?: string;
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                      <div className="flex items-start gap-4">
                        ...
                        <div className="space-y-1">
                          ...
                          <p className="text-sm font-bold text-gray-700">{req.message?.substring(0, 50)}...</p>
        ...
        <TabsContent value="replied">
          <Card className="border-none shadow-sm rounded-2xl p-24 text-center">
